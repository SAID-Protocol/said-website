...
interface HostNavbarProps {
  /** Hide scroll collapse behavior (useful for inner pages) */
  noCollapse?: boolean;
}
          ...
          {authenticated ? (
            <div className="nav-avatar-wrap" ref={avatarRef}>
              <button className="nav-avatar" onClick={() => setAvatarOpen(!avatarOpen)}>
                {user?.email?.address?.[0]?.toUpperCase() || user?.twitter?.username?.[0]?.toUpperCase() || '?'}
              </button>
              {avatarOpen && (
                <div className="nav-dropdown">
                  <a href="/dashboard" className="nav-dd-item">Dashboard</a>
                  <a href="/billing" className="nav-dd-item">Billing</a>
